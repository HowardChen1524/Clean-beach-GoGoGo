...
public class buildAndJoin_myEvent_finish extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.buildandjoin_myevent_finish);
                ...
                for(int i = 0; i < (int) dataSnapshot.child("User").getChildrenCount(); i++)
                    if (dataSnapshot.child("User").child("user" + i).child("user_email").getValue().toString().equals(user.getEmail())) {
                        for (int j = 0; j < (int) dataSnapshot.child("User").child("user" + i).child("user_join_event").getChildrenCount(); j++)
                            if (dataSnapshot.child("User").child("user" + i).child("user_join_event").child("join_event" + j).child("join_event_finish").getValue().toString().equals("finish")) {
    ...
    public void setRecycler_view(int userId, String eventName){
        ...
        // 將資料交給adapter
        adapter = new MyAdapter(this,new buildAndJoin_myEvent_finish_event(), allEventData,userId,eventName);
